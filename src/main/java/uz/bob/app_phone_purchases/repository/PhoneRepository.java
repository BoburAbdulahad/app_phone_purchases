...
public interface PhoneRepository extends JpaRepository<Phone,Long> {
    ...
    //===searching===/
    @Query(value = " select p.id,p.created_at,p.created_by,p.updated_at,p.updated_by,p.active,p.color,p.price,p.maker_id,p.photo_id from phone p join maker m on p.maker_id=m.id  where m.name='iPhone'",nativeQuery = true)
